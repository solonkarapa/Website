...
There are many tutorials for importing data in R, focusing on a specific function/package. This one focuses only on flat files, and compares 3 different packages. In this tutorial you will learn how to import all common formats of flat file data with base R functions and the dedicated `readr` and `data.table` packages. I first present the three - in my opinion - most used packages to import data into R and finish with a comparison table between them.  
...
## Concluding 
In this short tutorial we have seen three packages that can be used to import flat files into R, `utils`, `readr` and `data.table`. Each one has specific properties that make it unique.
