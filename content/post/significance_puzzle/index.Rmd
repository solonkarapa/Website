...
## Solution
To determine if Captain Kirk made the right decision, we need to calculate the probability that the ship is indeed Klingon, given Chekhov's identification. 

Let’s denote this probability, $\text{P(K given C)}$: the probability that the ship is Klingon (K) given Chekhov says it is Klingon (C).
...
$$\text{P(K given C)} = \frac{\text{P(C given K)⋅P(K)}}{\text{P(C given R)⋅P(R) + 
P(C given K)⋅P(K)}}$$

There is an extra unknown on the right-hand side, $\text{P(C given R)}$. This is the probability Chekhov misidentifies a Romulan ship as Klingon. 
