...
By multiplying these choices together to get our result: $4 \times 3 \times 2 (\times 1) = 24$. 

The actual formula behind the calculation is $n!/(n-r)!$, where $n$ is the number of things to choose from,
and we choose $r$ of them, with no repetitions (we need to choose each number only once). For us, $n= r= 4$ since we have $n=4$ buttons to choose from and we choose $r=4$ which gives $4!/(0!) = 4!/1 = 4 \times 3 \times 2 \times 1 = 24$.
...
where the asterisks indicate positions the letters can be repeated. For example, if $A$ is repeated then I simply replace the asterisks with $A$. We then get 
...
Note the first two are actually the same. So, in effect, two asterisks (shown in red above) are superfluous. This reduces the positions the letters can be repeated to two. And since we can repeat any of the three letters ($A$ or $B$ or $C$), the number of arrangements is $3! \times 2 \times 3 = 36$.
...
## Update 15/10/21
The [solution](https://www.newscientist.com/article/mg25133564-500-puzzle-135-how-do-you-hit-the-jackpot-on-this-machine/) from the New Scientist's.
