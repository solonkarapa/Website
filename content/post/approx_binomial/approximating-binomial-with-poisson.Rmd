...
This is the rate of success. That’s the number of trials $n$—however many there are—times the chance of success $p$ for each of those trials. In other words, if we repeat the experiment every day, we will be getting $\lambda$ successes per day on average. 
...
\begin{equation}
   (\#eq:limit)
   \frac{\lambda^x}{x!}  \lim_{n \to \infty} \frac{n!}{(n-x)!} \bigg( \frac{1}{n} \bigg)^x \bigg( 1-\frac{\lambda}{n} \bigg)^n \bigg( 1-\frac{\lambda}{n} \bigg)^{-x}
\end{equation}
...
$$\lim_{n \to \infty} \frac{n!}{(n-x)!} \bigg( \frac{1}{n} \bigg)^x$$
The numerator and denominator can be expanded as follows 

$$\lim_{n \to \infty} \frac{(n)(n-1)(n-2)\dots(n-x)(n-x-1)\dots (1)}{(n-x)(n-x-1)(n-x-2)\dots (1)}\bigg( \frac{1}{n} \bigg)^x$$
...
2. Now we focus on the middle term of \@ref(eq:limit)
...
$$\lim_{n \to \infty} \bigg( 1-\frac{\lambda}{n} \bigg)^n = \lim_{n \to \infty} \bigg( 1+\frac{1}{a} \bigg)^{-a\lambda} = e^{-\lambda}.$$
So we've finished with the middle term. 

3. The third term of \@ref(eq:limit) is 

$$ \lim_{n \to \infty}  \bigg( 1-\frac{\lambda}{n} \bigg)^{-x} $$
...
$$ \frac{\lambda^x}{x!}  \lim_{n \to \infty} \frac{n!}{(n-x)!} \bigg( \frac{1}{n} \bigg)^x \bigg( 1-\frac{\lambda}{n} \bigg)^n \bigg( 1-\frac{\lambda}{n} \bigg)^{-x} = \frac{\lambda^x}{x!} e^{-\lambda}$$
...
A [rule of thumb](https://www.itl.nist.gov/div898/handbook/pmc/section3/pmc331.htm) says for the approximation to be good: 
   ...
   > "The sample size $n$ should be equal to or larger than 20 and the probability of a single success, $p$, should be smaller than or equal to 0.05. If $n \geq $100, the approximation is excellent if $np$ is also $leq$ 10." 
