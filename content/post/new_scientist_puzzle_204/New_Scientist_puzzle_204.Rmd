...
5. Divide by 12 and subtract 4 - we call this t. Then, $t=s/12 - 4$
6. Subtract your original number - we call this u. That is, $u=t-x$. And, we know, $u=1$ (from step 7).

The puzzle states that the son “missed the final instruction to subtract this original number.” If he had subtracted the original number (after repeating one of the previous steps) he would have gotten a negative number (or zero). So we know, after repeatring a step, $t’ \leq 0$, where $t’$ is the result to step 5, after repeating one of the previous steps. 

Now, looking at steps 2 through 5, the only ones that can give us negative number are steps 4 and 5. Steps 2 and 3 involve the addition and multiplication of positive numbers - they cannot give us a negative result.

We are thus left with steps 4 and 5 as potential candidates. In fact, whatever the result after steps 2 and 3 we know that either: 

- (a) 2*step 4 + step 5 =1 (I call this proposition A) or 

- (b) step 4 and 2*step 5 = 1 (I call this proposition B)

We can check each of these propositions. 

Proposition A is: $(r-60)-60/12 -4 = 1$. I had simply repeated step 4 twice and then addede step 5. Solving for $r$ gives us $r = 180$. We can then backtrack, soliving for $q$ from steps 2 and 3 to get $x =5$. So $x= 5$ is a potential solution. 

Proposition 2 is: $(s/12 - 4)/12 - 4 = 1$ give us $s=708$. From steps 2-4  this implies that $x=54$, which is outside the range given in step 1. So this is not a valid solution. 

Summarizing, the only way to get a results equalt to 1 at step 6, after repeating one step twice and then missed the final instruction to subtract this original number, is to choose $x=5$. 


## Brute force solution
Another way to get a slution to this puzzle is to code steps 2 through 6, repeating each step twice and ehck the result for all number from 1 to 50. This is what the `magic_trick()` function does. 
