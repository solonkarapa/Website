...
QWERTY has become the dominant keyboard standard. The basic QWERTY form was developed in 1873 and was based around four rows with eleven characters in each row.
...
A common view is the letters QWERTY were assembled (on purpose) in one row so the salesman could impress the customers by quickly typing "typewriter" which was the name of the brand producing the hardware - "the Sholes and Glidden Type Writer".

> Using these six letters in QWERTY we can type the word “typewriter”. 

@kay2013rerun labels this view as  Myth 1.

They used probability theory to investigate whether this feature of the keyboard exists "by intent or accident". To do this, they calculated the probability the seven letters that make up "typewriter" falling on one line. This probability is 0.0002 which would indicate it was a design choice, rather than a random ordering of letters. 

Crucially, this calculation is based on the assumption that the designer had chosen in advance to place the 26 letter of the alphabet into 3 rows with 10 letters at the top row, 10 at the middle, and 6 at the bottom row[^1].

[^1]: In fact, later versions changed this arrangement to 10, 9, 7, a more balanced ordering - this the modern QWERTY.

Here, I’m looking how this probability changes for other values of letters/rows combinations.

# The calculation in @kay2013rerun
First, I briefly go through the calculations as presented in @kay2013rerun. The problem is parallel to sampling without replacement from an (imaginary) urn. The designer has chosen $K=10$ letters to be assigned at the top row of the keyboard and the rest $R=26-10=16$ to the other rows (there are $N=26$ letters in the alphabet).
...
The hypergeometric distribution describes the probability of $k=7$ "successes" in $n=7$ draws, without replacement, from a finite population of size $N = 26$ letters that contains exactly K = 10 objects with that feature, wherein each draw is either a success or a failure. Using the usual urn-style "language" of "green" and "red" marbles we have: 
 ...
 - $k = 7$ the letters that form the word "typerwriter"
...
Now we are interested in the probability of drawing "$k=7$ green marbles in 7 draws for different choices of $K$ (and $R$ consequently)". The plots shows the probability of $k=7$ for different values of $K$. We start with $K = 6$ which gives Probability = 0, as there is no way to draw 7 green balls when there are only 6 of them! The red dot corresponds to $K=10$ which has been used in the paper. For $K=19$ which corresponds to the quite extreme scenario of having 19 letters at the top row, the probability the 7 "typerwriter" letters are found there becomes 8%. Still small but appreciable.   
